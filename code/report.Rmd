...
# Best models based on the deal package structure learning

```{r echo=FALSE}
c <- deal_best_models %>% count(model) %>% arrange(desc(n))
invisible(apply(c,1,function(x){
  perc <- round(as.numeric(x["n"]) / sum(c$n) * 100,2)
  plot(model2network(as.character(x["model"])),main = paste0(perc," %"," (n =",x["n"],")"," with this structure"))
}))
```
