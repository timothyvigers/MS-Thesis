...
kable(cinf)
# Permutation tests
cinf = cit.bp(L = X[,1],G = X[,2],T = y,n.perm = 100,rseed = 1017)
colnames(cinf) = gsub("L","methyl",colnames(cinf))
colnames(cinf) = gsub("G","metab",colnames(cinf))
colnames(cinf) = gsub("T","T1D",colnames(cinf))
colnames(cinf) = gsub("assoc"," ~ ",colnames(cinf))
...
cits = apply(pairs[1:100,], 1, function(x){
  ...
  if (cinf1$p_cit < 0.05 | cinf2$p_cit < 0.05) {
    return(c(methyl,metab,min(cinf1$p_cit,cinf2$p_cit)))
  } else {return(NA)}
})
# Convert to dataframe
cits = cits[!is.na(cits)]
cits = t(as.data.frame(cits))
# Summarize
