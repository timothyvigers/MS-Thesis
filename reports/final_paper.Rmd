...
Pathway over-representation was examined using 60 unique genes associated with the top 100 probes using PANTHER's[@miPANTHERVersion162021] implementation of Reactome pathways[@fabregatReactomePathwayKnowledgebase2018]. These p values were based on standard error of NIE estimated using the multivariate delta method.[@vanderweeleExplanationCausalInference2015] CpG sites were converted to Entrez IDs using the missMethyl R package[@phipsonMissMethylPackageAnalyzing2016a] and the 199,243 original probes were used as the reference list. Pathways with less than 5 genes in the reference list and less than two genes in analysis list were excluded.

Pathway enrichment analysis was performed using the Enrichr (https://maayanlab.cloud/Enrichr/)[@xieGeneSetKnowledge2021] Biopathways 2019 module[@huangNCATSBioPlanetIntegrated2019], again with 60 unique genes based on the top 100 candidate probes. 
...
A flowchart depicting progression from birth to islet autoimmunity (IA) and the Diabetes Autoimmunity Study in the Young (DAISY) study visits (a). To reduce computation time, we performed simple linear regression without covariate adjustment to identify pairs of DNA methylation (DNAm) probes and metabolites for mediation analysis. Candidate pairs were significantly associated at the p < 0.01 level, and both DNAm probe and metabolite were associated with development of IA at the p < 0.01 level. We examined pairs with DNAm measured pre-seroconversion (PSV) and metabolite measured at seroconversion (SV) (b) and pairs with metabolite measured at PSV and DNAm measured at SV (c) in separate analyses.
...
colnames(t) = c("Pathway","# in Reference List","# Genes in Top 100 Probes","Expected","Fold Enrichment","p value")
t = t %>% filter(`# in Reference List` >= 5 & `# Genes in Top 100 Probes` >= 2)
...
bioplanet = bioplanet %>% 
  ...
  filter(Ref >= 2) %>%
  rename(`# in Reference List` = Ref,`# Genes in Top 100 Probes` = Background,
