...
colnames(reactome) = c("Pathway","# in Reference List","# Genes in Top 100 Probes","Expected","Fold Enrichment","p value")
reactome = reactome %>% filter(`# in Reference List` >= 5 & `# Genes in Top 100 Probes` >= 2)
reactome[,"FDR q value"] = p.adjust(reactome$`p value`,"fdr")
kable(reactome[reactome$`FDR q value` < 0.1,],digits = 3)
```
...
```{r}
# Import reactome results
react_results <- fromJSON(file='./results/reactome_results.json')
# For each pathway, grep genes
entrez_ids = lapply(reactome$Pathway, function(g){
  i = grep(sub(" \\(R-HSA-.*","",g),react_results$overrepresentation$result)[1]
  react_results$overrepresentation$result[[i]]$input_list$mapped_id_list$mapped_id
})
# Get probe IDs
```
