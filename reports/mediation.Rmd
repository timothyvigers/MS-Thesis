...
```{r}
load("./data/mediation/cits_mediation_sensitivity.Rdata")
load("./data/mediation/cits_mediation.Rdata")
cits_mediation = as.data.frame(cits_mediation)
cits_mediation[,3:ncol(cits_mediation)] = lapply(cits_mediation[,3:ncol(cits_mediation)],as.numeric)
# Plot
data <- c(0, "'a'", 0,
          0, 0, 0, 
          "'b'", "'c'", 0)
...
```{r}
methyl_med = as.data.frame(cits_mediation[grep("cg.*",cits_mediation[,"M"]),])
metab_med = as.data.frame(cits_mediation[grep("cg.*",cits_mediation[,"X"]),])
```
...
```{r}
ggplot(cits_mediation,aes(x = `Average ACME`, y = -log10(`Average ACME p`))) +
  geom_point() + theme_bw()
```
...
```{r}
s = cits_mediation_sensitivity[[1]][[1]]
plot(s,sens.par = "rho")
plot(s,sens.par = "R2")
summary(s)
```
