...
```{r results='asis'}
# Panther
t = read.delim("./results/panther_probe_background.txt")
t = t %>% arrange(final_probes_entrez_ids.csv..raw.P.value.)
kable(head(t,10),caption = "Panther (Probe Background)")

t = read.delim("./results/panther_generic_background.txt")
t = t %>% arrange(final_probes_entrez_ids.csv..raw.P.value.)
kable(head(t,10),caption = "Panther (Generic Background)")
# Reactome
t = read.delim("./results/reactome_probe_background.txt")
t = t %>% arrange(final_probes_entrez_ids.csv..raw.P.value.)
kable(head(t,10),caption = "Reactome (Probe Background)")

t = read.delim("./results/reactome_generic_background.txt")
t = t %>% arrange(final_probes_entrez_ids.csv..raw.P.value.)
...
Again, the 9 significant probes were used for Enrichr analysis, the results of which can be found here: <https://maayanlab.cloud/Enrichr/enrich?dataset=1e23019346c30f1b9edbf92e9dce4d64>. The Bioplanet 2019 results here look particularly interesting, but it's tough to say how much to read into them.
