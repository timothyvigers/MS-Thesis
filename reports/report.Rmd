...
names(res) = names(mediation_mods)
res = res[!is.na(res)]
...
res = lapply(names(mediation_mods), function(n){
  s = summary(mediation_mods[[n]])
  if((!is.na(s$n.avg) & s$n.avg > 0.05) & (!is.na(s$n.avg.p) & s$n.avg.p < 0.05)){
    return(s)
  } else {
    return(NA)
  }
...
Of the `r nrow(methyl_psv_candidates)` candidate pairs, `r length(res)` had an average proportion mediated > 5% (statistically significant a nominal 0.05 level):
